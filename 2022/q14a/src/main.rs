...
impl Sand {
    ...
    fn fall(&mut self, rocks: &mut HashSet::<(i32,i32)>) -> bool {
    ...
    fn is_abyss(&self, y_max: i32) -> bool {
        self.y > y_max
    }
}
...
fn main() {
    ...
    let y_max = rocks.iter().map(|r| r.0).max().unwrap();
    let mut sand = Sand::new();
    let mut c: usize = 0;
    while !sand.is_abyss(y_max) {
        while sand.fall(&mut rocks){
            if sand.is_abyss(y_max) {
               break; 
            }
        }
        if !sand.is_abyss(y_max) {
